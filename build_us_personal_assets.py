...
Build personal US assets from us_personal_batch_*.json:
  - infographic PNGs (us-personal-images/DATE/day-NN.png)
...
SOURCE = "Hitesh Dodiya · AI Automation"
BRAND = "AI Automation"
SITE = "Comment AUTO"
...
def render_infographic(spec, out_html):
    ...
    html = html.replace("{{TITLE_MAIN}}", spec.get("title_main", "Automate the"))
    html = html.replace("{{TITLE_SPAN}}", spec.get("title_span", "busywork"))
    ...
    html = html.replace(">Follow me<", ">Follow for automations<")
...
def build_slide_html(s, num):
    if s.get("cta"):
        ...
        bottom = '<div class="pill">Comment <em>AUTO</em> to start.</div>'
    else:
        main = (
