...
Build BookWellNow assets from bookwellnow_batch_*.json:
  - infographic PNGs (bookwellnow-images/DATE/day-NN.png)
...
def render_infographic(spec, out_html):
    ...
    html = INFO_TMPL
    html = html.replace("{{BADGE}}", spec.get("badge", "BookWellNow"))
    html = html.replace("{{DATE_LABEL}}", spec.get("date_label", datetime.date.today().strftime("%B %Y")))
    html = html.replace("{{TITLE_MAIN}}", spec.get("title_main", "Take bookings"))
    html = html.replace("{{TITLE_SPAN}}", spec.get("title_span", "online"))
    ...
    # Shared template ships a personal-brand footer CTA
    html = html.replace(">Follow me<", ">Follow BookWellNow<")
...
<div class="hright"><div class="meta">bookwellnow.com</div><div class="badge">{num}</div></div></div>
{main}
...
def build_slide_html(s, num):
    if s.get("cta"):
        ...
        bottom = '<div class="pill">Comment <em>BOOK</em> to start.</div>'
    else:
        main = (
...
for i, day in enumerate(posts, 1):
    ...
    car_name = f"bookwellnow-day-{i:02d}"
