...
Build Digital Card Creator assets from digitalcard_batch_*.json:
  - infographic PNGs (digitalcard-images/DATE/day-NN.png)
  - carousels (HTML → PNG → PDF via carousel-routine)
"""
import datetime
import glob
import json
...
if os.environ.get("DIGITALCARD_BATCH"):
    BATCH = os.environ["DIGITALCARD_BATCH"]
    if not os.path.exists(BATCH):
        sys.exit(f"DIGITALCARD_BATCH not found: {BATCH}")
elif not files:
...
def render_infographic(spec, out_html):
    ...
    html = INFO_TMPL
    html = html.replace("{{BADGE}}", spec.get("badge", BRAND))
    html = html.replace("{{DATE_LABEL}}", spec.get("date_label", datetime.date.today().strftime("%B %Y")))
    html = html.replace("{{TITLE_MAIN}}", spec.get("title_main", "Share smarter"))
    html = html.replace("{{TITLE_SPAN}}", spec.get("title_span", "digital cards"))
    ...
    html = html.replace("{{SOURCE}}", spec.get("source", SOURCE))
    html = html.replace("#5E6AD2", ACCENT)
    html = html.replace(">Follow me<", ">Follow Digital Card Creator<")
...
.fw{{font-family:'Instrument Serif',serif;font-style:italic;font-size:22px;color:#999}}
...
<div class="hright"><div class="fw">{site}</div><div class="badge">{num}</div></div></div>
{main}
...
def build_slide_html(s, num):
    if s.get("cta"):
        ...
        bottom = '<div class="pill">Comment <em>CARD</em> to start.</div>'
    else:
        main = (
...
for i, day in enumerate(posts, 1):
    ...
    car_name = f"digitalcard-day-{i:02d}"
